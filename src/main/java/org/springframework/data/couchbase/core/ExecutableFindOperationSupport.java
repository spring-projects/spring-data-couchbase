...
class ExecutableFindOperationSupport implements ExecutableFindOperation {
  ...
  ExecutableFindOperationSupport(CouchbaseTemplate template) {
  ...
  public <T> ExecutableFind<T> findById(Class<T> domainType) {
    return new ExecutableFindSupport<>(template, domainType, null, null, null, ALL_QUERY);
  }
  /*
   * (non-Javadoc)
  ...
  public <T> ExecutableFind<T> query(Class<T> domainType) {
    ...
    return new ExecutableFindSupport<>(template, domainType, domainType, null,null, ALL_QUERY);
  }
  ...
  static class ExecutableFindSupport<T>
    ...
    ExecutableFindSupport(CouchbaseTemplate template, Class<?> domainType, Class<T> returnType,
                          String scope, String collection, Query query) {
      this.template = template;
      ...
      this.query = query;
      this.scanConsistency = QueryScanConsistency.REQUEST_PLUS;
      this.reactiveSupport = new ReactiveFindByQueryOperationSupport.ReactiveFindByQuerySupport<T>(template.reactive(), (Class <T>)domainType, query, scanConsistency, scope, collection);
    }
     ...
     */
    @Override
    public FindWithProjection<T> inScope(String scope) {

      Assert.hasText(collection, "Collection name must not be null nor empty!");

      return new ExecutableFindSupport<>(template, domainType, returnType, scope, collection, query);
    }
    /*
     * (non-Javadoc)
     * @see org.springframework.data.mongodb.core.ExecutableFindOperation.FindWithCollection#inCollection(java.lang.String)
     */
    @Override
    public FindWithProjection<T> inCollection(String scope, String collection) {
    ...
    public <T1> FindWithQuery<T1> as(Class<T1> returnType) {
      ...
      return new ExecutableFindSupport<T1>(template, domainType, returnType, scope, collection, query);
    }
     ...
     */
    @Override
    public List<T> all(Collection<String> ids) {
      //return /* TODO doFind(null) */;
    ...
    public Stream<T> stream() {
      return null /* TODO StreamUtils.createStreamFromIterator(doStream())*/;
    }
     ...
     */
    @Override
    public TerminatingFindNear<T> near(Object nearQuery) {
      return null; // TODO () -> template.geoNear(nearQuery, domainType, getCollectionName(), returnType);
    }
...
*/
  ...
  */
  ...
  static class DistinctOperationSupport<T> implements ExecutableFindOperation.TerminatingDistinct<T> {
    ...
    private final ExecutableFindOperationSupport.ExecutableFindSupport<T> delegate;

    public DistinctOperationSupport(ExecutableFindOperationSupport.ExecutableFindSupport<T> delegate, String field) {
    ...
    public <R> ExecutableFindOperation.TerminatingDistinct<R> as(Class<R> resultType) {
      ...
      return new DistinctOperationSupport<>((ExecutableFindOperationSupport.ExecutableFindSupport) delegate.as(resultType), field);
    }
    ...
    public ExecutableFindOperation.TerminatingDistinct<T> matching(Query query) {
      ...
      return new DistinctOperationSupport<>((ExecutableFindOperationSupport.ExecutableFindSupport<T>) delegate.matching(query), field);
    }
    ...
    @Override
    public List<T> all() {
      return null /* TODO delegate.doFindDistinct(field) */;
    }
  }
}
