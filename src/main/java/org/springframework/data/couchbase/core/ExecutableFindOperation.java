...
//import org.springframework.data.couchbase.core.query.NearQuery;
import org.springframework.data.couchbase.core.query.Query;
...
public interface ExecutableFindOperation {
   ...
   * @throws IllegalArgumentException if domainType is {@literal null}.
   */
  <T> ExecutableFind<T> findById(Class<T> domainType);
  ...
  interface TerminatingFind<T> {
     ...
     */
    java.util.Collection<? extends T> all(java.util.Collection<String> ids);
  }
  ...
  interface FindWithQuery<T> extends TerminatingFind<T> {
    ...
    /**
     * Set the filter {@link QueryCriteria criteria} to be used.
     *
     ...
     * @since 3.0
     */
    default TerminatingFind<T> matching(QueryCriteria criteria) {
     ...
     */
    TerminatingFindNear<T> near(Object nearQuery);
  }
  ...
  interface FindWithCollection<T> extends FindWithQuery<T> {
     ...
     */
    FindWithProjection<T> inCollection(String scope, String collection);
    FindWithProjection<T> inScope(String scope);
  ...
  interface FindDistinct {
    ...
    /**
     * Finds the distinct values for a specified {@literal field} across a single {@link Collection} or view.
     *
  ...
  interface DistinctWithQuery<T> extends DistinctWithProjection {
     ...
     * @since 3.0
     */
    default TerminatingDistinct<T> matching(QueryCriteria criteria) {
      return matching(Query.query(criteria));
    }
  }
