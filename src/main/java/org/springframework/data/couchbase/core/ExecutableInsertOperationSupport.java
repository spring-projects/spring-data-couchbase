...
class ExecutableInsertOperationSupport implements ExecutableInsertOperation {
	...
	ExecutableInsertOperationSupport(CouchbaseTemplate template) {
	...
	public <T> ExecutableInsert<T> insert(Class<T> domainType) {
		...
		return new ExecutableInsertSupport<T>(template, domainType, null, false /* null */);
	}
	...
	static class ExecutableInsertSupport<T> implements ExecutableInsert<T> {
		...
		@Nullable private final /*BulkMode*/ boolean bulkMode;

		ExecutableInsertSupport(CouchbaseTemplate template, Class<T> domainType, String collection, boolean /*BulkMode*/ bulkMode) {
		...
		public T one(T object) {
			...
			return null; // template.insertById(object.getClass()/*, getCollectionName()*/) // .one((Object)object);
		}
		...
		public Collection<T> all(Collection<? extends T> objects) {
			...
			return null; // template.insertById(objects.getClass()).all(objects); // objects, getCollectionName());
		}
		 ...
		 */
		@Override
		public /*BulkWriteResult*/ Object bulk(Collection<? extends T> objects) {

			Assert.notNull(objects, "Objects must not be null!");

			return null; // template.bulkOps(bulkMode != null ? bulkMode : BulkMode.ORDERED, domainType, getCollectionName())
		 ...
		 */
		@Override
		public TerminatingBulkInsert<T> withBulkMode(boolean /*BulkMode*/ bulkMode) {
		...
		private String getCollectionName() {
			return StringUtils.hasText(collection) ? collection : null /*template.getCollectionName(domainType)*/;
		}
	}
}
