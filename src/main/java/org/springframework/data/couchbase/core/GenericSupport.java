...
class GenericSupportHelper {
    ...
    public GenericSupportHelper(CouchbaseDocument doc, Long cas, Collection collection, @Nullable TransactionAttemptContext ctx) {
        this.converted = doc;
        this.cas = cas;
        ...
        return template.getCouchbaseClientFactory().withScope(scopeName).getCollection(collectionName)
                .flatMap(collection ->
                        support.encodeEntity(object)
                                .flatMap(converted -> {
                                    GenericSupportHelper gsh = new GenericSupportHelper(converted, support.getCas(object), collection, ctxr.orElse(null));
                                    if (!ctxr.isPresent()) {
