 ...
 * <p/>
 * The statement can contain positional placeholders (eg. <code>name = $1</code>) that will map to the method's parameters, in the same order.
 * <p/>
 * The statement can also contain SpEL expressions enclosed in <code>#{</code> and <code>}</code>.
 * <p/>
 * There are couchbase-provided variables included for the {@link #SPEL_BUCKET bucket namespace}, the {@link #SPEL_ENTITY ID and CAS fields} necessary for entity reconstruction or a shortcut that
 * covers {@link #SPEL_SELECT_FROM_CLAUSE SELECT AND FROM clauses}, along with a variable for {@link #SPEL_FILTER WHERE clause filtering} of the correct entity.
 *
 * @author Simon Baslé
 */
public class StringN1qlBasedQuery extends AbstractN1qlBasedQuery {
    ...
    /**
     * Use this variable in a SpEL expression in a {@link org.springframework.data.couchbase.core.query.Query @Query} annotation's inline statement. This will be replaced by the correct
     * <code>SELECT x FROM y</code> clause needed for entity mapping. Eg. <code>"#{{@value StringN1qlBasedQuery#SPEL_SELECT_FROM_CLAUSE}} WHERE test = true"</code>. Note this only makes sense once, as
     * the beginning of the statement.
     */
    public static final String SPEL_SELECT_FROM_CLAUSE = "#" + SPEL_PREFIX + ".selectEntity";

    /**
     * Use this variable in a SpEL expression in a {@link org.springframework.data.couchbase.core.query.Query @Query} annotation's inline statement. This will be replaced by the (escaped) bucket name
     * corresponding to the repository's entity. Eg. <code>"SELECT * FROM #{{@value StringN1qlBasedQuery#SPEL_BUCKET}} LIMIT 3"</code>.
     */
    public static final String SPEL_BUCKET = "#" + SPEL_PREFIX + ".bucket";

    /**
     * Use this variable in a SpEL expression in a {@link org.springframework.data.couchbase.core.query.Query @Query} annotation's inline statement. This will be replaced by the fields allowing to
     * construct the repository's entity (SELECT clause). Eg. <code>"SELECT #{{@value StringN1qlBasedQuery#SPEL_ENTITY}} FROM test"</code>.
     */
    public static final String SPEL_ENTITY = "#" + SPEL_PREFIX + ".fields";

    /**
     * Use this variable in a SpEL expression in a {@link org.springframework.data.couchbase.core.query.Query @Query} annotation's inline statement WHERE clause. This will be replaced by the
     * expression allowing to only select documents matching the entity's class. Eg. <code>"SELECT * FROM test WHERE test = true AND #{{@value StringN1qlBasedQuery#SPEL_FILTER}}"</code>.
     */
    ...
    /**
     * Parse the statement to detect SPEL blocks (delimited by <code>#{</code> and <code>}</code>) and replace said expression blocks with their corresponding values.
     *
    ...
    /**
     * This class is exposed to SpEL parsing through the variable <code>#{@value StringN1qlBasedQuery#SPEL_PREFIX}</code>. Use the attributes in your SpEL expressions: {@link #selectEntity},
     * {@link #fields}, {@link #bucket} and {@link #filter}.
     */
