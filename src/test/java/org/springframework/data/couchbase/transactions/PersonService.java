...
class PersonService {
  ...
  public PersonService(CouchbaseOperations ops, CouchbaseTransactionManager mgr, ReactiveCouchbaseOperations opsRx,
    ...
    manager = mgr;
    System.err.println("operations cluster  : " + personOperations.getCouchbaseClientFactory()
        .getCluster());
    System.err.println("manager cluster     : " + manager.getDatabaseFactory()
        .getCluster());
    ...
    managerRx = mgrRx;
    System.out
        .println("operationsRx cluster  : " + personOperationsRx.getCouchbaseClientFactory()
            .getCluster()
            .block());
    System.out.println("managerRx cluster     : " + mgrRx.getDatabaseFactory()
        .getCluster()
        .block());
  ...
  public Person declarativeFindReplacePersonCallback(Person person, AtomicInteger tryCount) {
    ...
    System.err.println("declarativeFindReplacePersonCallback resourceHolder : "
        ...
        .getResource(callbackTm.template()
            .getCouchbaseClientFactory()
            .getCluster()
            .block()));
  ...
  void assertInAnnotationTransaction(boolean inTransaction) {
    ...
    for (StackTraceElement ste : stack) {
      if (ste.getClassName()
          .startsWith("org.springframework.transaction.interceptor") ||
          ste.getClassName()
              .startsWith("org.springframework.data.couchbase.transaction.interceptor")) {
        if (inTransaction) {
          return;
        }
      }
    ...
    throw new RuntimeException(
        "in_annotation_transaction = " + (!inTransaction) + " but expected in_annotation_transaction = " + inTransaction + "\n class: " + getClass().getName());
  }
