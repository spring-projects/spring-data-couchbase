...
class PersonServiceReactive {
  ...
  public PersonServiceReactive(CouchbaseOperations ops, /* CouchbaseCallbackTransactionManager mgr, */ ReactiveCouchbaseOperations opsRx,
                               ReactiveCouchbaseTransactionManager mgrRx) {
    personOperations = ops;
    personOperationsRx = opsRx;
    managerRx = mgrRx;
    transactionalOperator = TransactionalOperator.create(managerRx,
  ...
  public Flux<CouchbasePersonTransactionReactiveIntegrationTests.EventLog> saveWithLogs(Person person) {
    return Flux.merge(personOperationsRx.save(new CouchbasePersonTransactionReactiveIntegrationTests.EventLog(new ObjectId().toString(),
            "beforeConvert")),
        //
        personOperationsRx.save(new CouchbasePersonTransactionReactiveIntegrationTests.EventLog(new ObjectId(),
            "afterConvert")),
        //
        personOperationsRx.save(new CouchbasePersonTransactionReactiveIntegrationTests.EventLog(new ObjectId(),
            "beforeInsert")),
  ...
  public Flux<Void> saveWithErrorLogs(Person person) {
    return Flux.merge(personOperationsRx.save(new CouchbasePersonTransactionReactiveIntegrationTests.EventLog(new ObjectId(),
            "beforeConvert")),
        //
        personOperationsRx.save(new CouchbasePersonTransactionReactiveIntegrationTests.EventLog(new ObjectId(),
            "afterConvert")),
  ...
  public Flux<Person> declarativeSavePersonErrors(Person person) {
    Person p = personOperations.insertById(Person.class)
        .one(person);
    Person pp = personOperations.findByQuery(Person.class)
        .withConsistency(REQUEST_PLUS)
        .all()
        .get(0);
